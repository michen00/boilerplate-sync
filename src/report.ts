...
export function generatePrBody(summary: SyncSummary, schedule?: string): string {
  ...
  if (summary.created.length > 0) {
    lines.push(`### 🆕 Created (${summary.created.length} file${summary.created.length === 1 ? '' : 's'})`);
    lines.push('');
    lines.push('| File | Source | Ref |');
    lines.push('|------|--------|-----|');
  ...
  if (summary.skipped.length > 0) {
    lines.push(`### ⏭️ Skipped (${summary.skipped.length} file${summary.skipped.length === 1 ? '' : 's'})`);
    lines.push('');
  ...
  if (summary.failed.length > 0) {
    lines.push(`### ❌ Failed (${summary.failed.length} file${summary.failed.length === 1 ? '' : 's'})`);
    lines.push('');
    ...
    if (summary.allFailed) {
      lines.push('> ⚠️ **All files failed to sync.** This PR is marked as draft. Please investigate the errors above.');
      lines.push('');
    }
  }
...
export function generateStepSummary(summary: SyncSummary): string {
  ...
  if (summary.hasChanges) {
    lines.push('✅ Changes detected - PR will be created');
  } else if (summary.allFailed) {
    lines.push('⚠️ All files failed - draft PR will be created');
  } else {
    lines.push('ℹ️ No changes detected - no PR needed');
  }
