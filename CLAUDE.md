...
- **Never hand-edit CHANGELOG.md**: it is generated by release-please; manual edits get overwritten
- **CHANGELOG.md is in `.prettierignore` on purpose**: "fixing" its `*` bullets or double blank lines breaks the required `Pre-commit hooks` check on every future release PR
- **prettier does not format markdown**: `*.md` is in `.prettierignore` because prettier only emits *aligned* tables while `.markdownlint.yml` requires `compact` ones, so letting both touch markdown makes the two hooks rewrite each other forever. Fix markdown with `markdownlint --fix`; never hand markdown back to prettier
